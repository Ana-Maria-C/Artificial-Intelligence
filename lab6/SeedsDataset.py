...
class SeedsDataset:
    def __init__(self, file_path):
        self.x_train, self.x_test, self.y_train, self.y_test = self.read_and_split_data(file_path)
        self.y_train = self.y_train - 1
        self.y_test = self.y_test - 1
        self.parameters = self.initialize_parameters()

    def data_split(self, data, test_size=0.2, random_seed=None):
    ...
    def read_and_split_data(self, file_path):
        ...
        x_train, x_test, y_train, y_test = self.data_split(data, test_size=0.2, random_seed=42)
    ...
    def backward_propagation(self, x, y, cache):
        ...
        dz2 = np.dot(dz3, w3.T) * self.sigmoid_derivative(z2)
        dw2 = 1 / x.shape[0] * np.dot(a1.T, dz2)
        db2 = 1 / x.shape[0] * np.sum(dz2, axis=0, keepdims=True)

        dz1 = np.dot(dz2, w2.T) * self.sigmoid_derivative(z1)
    ...
    def train(self):
        ...
        for epoch in range(max_epochs):
            ...
            if epoch % 100 == 0:
                print(f"Cost după epoca {epoch}: {cost}")
