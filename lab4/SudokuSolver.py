class SudokuSolver:
    ...
    def revise(self, row, col):
        for i in range(9):
            if i != col and (self.grid[row][i] == 0 or self.grid[row][i] == -1):
                if not any(self.is_consistent(row, i, val) for val in self.domain):
                    return True
            if i != row and (self.grid[i][col] == 0 or self.grid[i][col] == -1):
                if not any(self.is_consistent(i, col, val) for val in self.domain):
                    return True
        start_row, start_col = 3 * (row // 3), 3 * (col // 3)
        for i in range(start_row, start_row + 3):
            for j in range(start_col, start_col + 3):
                if i != row and j != col and (self.grid[i][j] == 0 or self.grid[i][j] == -1):
