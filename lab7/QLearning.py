...
class QLearning:
    def __init__(self, num_rows, num_cols, start_state, goal_state, wind, num_actions=4, alpha=0.5, gamma=0.9, epsilon=0.1 ):
    ...
    def update_table(self, state, action, reward, next_state):
        self.Q[state[0], state[1], action] += self.alpha * (
                    reward + self.gamma * np.max(self.Q[next_state[0], next_state[1], :]) - self.Q[
