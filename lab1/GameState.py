class GameState:
    ...
    def make_move(self, value_of_position):
        if self.is_valid_move(value_of_position):
            ...
            self.vect[index_zero], self.vect[index_value], self.vect[0] = self.vect[index_value], self.vect[index_zero], \
            self.vect[index_value]

    def index(self, value_of_position):
        for i in range(len(self.vect)):
            if self.vect[i] == value_of_position:
                return i
        return -1class GameState:
    def __init__(self, vect, moves=[]):
        if len(vect) == 9:
            self.vect = vect
            self.vect.insert(0, 9)
    ...
    def iddfs(self):
        ...
        while True:
            ...
            result = self.dfs(0, max_depth)
            if result is not None:
                return result, max_depth, self.moves
            max_depth += 1
    ...
    def dfs(self, depth, max_depth):
        ...
        for i in range(1, 9):
            if self.is_valid_move(i):
                self.make_move(i)
                result = self.dfs(depth + 1, max_depth)
    ...
    def greedy_search(self, heuristic_function):
        ...
        while priority_queue:
            ...
            next_states = [node.generate_next_state(value) for value in next_moves if
                           tuple(node.generate_next_state(value).vect[1:]) not in visited_states]
            next_states.sort(key=lambda x: heuristic_function(x))
    ...
    def generate_next_state(self, value_of_position):
        new_moves = self.moves + [value_of_position]
